...
type LineOverrideState = Record<string, string>
...
export default function CheckoutPage() {
  ...
  const pricingLines = useMemo<PricingLine[]>(() => {
    return cart.map((item, index) => {
      ...
      const uniqueLineId = `${lineKey}:${index}`
      ...
      const overrideValue = lineOverrides[uniqueLineId]
  ...
  const handlePayment = async () => {
    ...
    if (paymentMethod === "maishapay") {
      ...
      try {
        const receipt = `MP-${Date.now()}`
        const response = await backendRequest<BackendResponse<{ id?: number; reference?: string; redirect_url?: string }>>("/maishapay/session", {
          method: "POST",
          body: JSON.stringify({
            amount: Number(paymentAmountNum.toFixed(decimalsSelected)),
            currency_code: selectedCurrency?.code ?? "CDF",
            receipt,
            devise_vente_id: Number(selectedCurrencyId),
            mode_paiement: "card",
            frais_transaction: 0,
            payload: {
              ...
              lignes: pricingLines.map((line) => ({
                ...
                id_variante_produit: line.variantId,
                quantite: line.quantity,
                prix_vente: Number(line.finalUnitPrice.toFixed(decimalsSelected)),
                id_devise: Number(selectedCurrencyId),
              })),
              paiements: (
                useSplitPayment
                  ? [
        ...
        if (!redirectUrl) {
          throw new Error("Aucune redirection MaishaPay reçue.")
        }
        ...
        setPaymentError({
          title: "Paiement MaishaPay refusé",
          description: error instanceof Error ? error.message : "La session MaishaPay n'a pas pu être créée.",
        })
      } finally {
                ...
                <RadioGroup value={paymentMethod} onValueChange={setPaymentMethod}>
                  <div
                    className={`flex items-center space-x-2 rounded-lg border p-3 cursor-pointer transition-colors sm:p-4 ${
                      paymentMethod === "maishapay" ? "border-primary bg-primary/5" : ""
                    }`}
                  >
                    <RadioGroupItem value="maishapay" id="maishapay" />
                    <Label htmlFor="maishapay" className="flex items-center cursor-pointer flex-1">
