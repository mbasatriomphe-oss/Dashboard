...
function formatVariantLabel(value) {
  ...
  if (typeof value === "string") {
    try {
      ...
      if (parsed && typeof parsed === "object") {
        const values = Object.values(parsed).filter((entry) => entry !== null && entry !== undefined && entry !== "")
  ...
  if (Array.isArray(value)) {
    const items = value.filter((entry) => entry !== null && entry !== undefined && entry !== "")
    return items.length > 0 ? items.join(" / ") : "Standard"
  }

  if (typeof value === "object") {
    const values = Object.values(value).filter((entry) => entry !== null && entry !== undefined && entry !== "")
    return values.length > 0 ? values.join(" / ") : "Standard"
  }
...
function getMovementQuantity(movement) {
  const quantity = safeNumber(movement?.quantite)
  const type = movement?.type_mouvement

  if (type === "sortie") return -quantity
  return quantity
}
...
export default function StockReportsPage() {
  ...
  useEffect(() => {
    ...
    const productIds = [...new Set(stockData.map((product) => Number(product.id)).filter((id) => Number.isFinite(id) && id > 0))]
    ...
    const loadMovements = async () => {
      setMovementLoadingByProduct((current) => {
        const next = { ...current }
      ...
      try {
        const results = await Promise.all(
          productIds.map(async (productId) => {
            try {
              const response = await backendRequest(`/mouvements-stock-fifos/produit/${productId}`)
              const movements = normalizeRows(response).map((movement) => ({
                ...
                quantitySigned: getMovementQuantity(movement),
                balanceBefore: safeNumber(movement.quantite_restante_avant),
                balanceAfter: safeNumber(movement.quantite_restante_apres),
                lotNumber: movement.numero_lot || `LOT-${movement.id_lot ?? ""}`,
                lotId: movement.id_lot ?? null,
                variantLabel: formatVariantLabel(movement.variante_combinaison ?? movement.variante ?? movement.combinaison ?? movement.variante_code),
                variantCode: movement.variante_code || movement.code_sku || null,
                origin: movement.id_ligne_vente ? "Vente" : movement.id_ligne_retour ? "Retour" : "Approvisionnement",
                productName: movement.produit_nom || movement.product_name || "Produit",
              }))
              return [productId, movements]
            } catch {
              return [productId, []]
            }
          })
        ...
        if (!cancelled) {
          setMovementByProduct((current) => {
            const next = { ...current }
            results.forEach(([productId, movements]) => {
              next[productId] = movements
            })
            return next
          })
        }
  ...
  const getMovementsForProduct = (productId) => {
    const productMovements = movementByProduct[productId] ?? []
    const rangeDays = Math.max(1, Number(dateRange) || 30)
    const cutoff = new Date(Date.now() - rangeDays * 24 * 60 * 60 * 1000)
            ...
            <CardContent>
              <div className="space-y-4">
                {filteredData.map((product) => {
                  const productMovements = getMovementsForProduct(product.id)
                            ...
                            <div className="space-y-4">
                              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                ...
                                <div className="rounded-lg bg-red-50 p-3 dark:bg-red-900/20">
                                  ...
                                  <p className="text-lg font-bold text-red-700 dark:text-red-300">-{totalExits}</p>
                                </div>
                                <div className="rounded-lg bg-slate-100 p-3 dark:bg-slate-800/60">
                                  <p className="text-xs text-muted-foreground">Stock actuel</p>
                                  <p className="text-lg font-bold text-foreground">{product.currentStock}</p>
                                </div>
                              </div>
                              ...
                              <Table>
                                ...
                                <TableBody>
                                  {productMovements.slice(0, 10).map((movement, idx) => (
                                    <TableRow key={movement.id ?? `${movement.date}-${movement.type}-${idx}`}>
                                      <TableCell>{movement.date}</TableCell>
                                      <TableCell className="font-medium">{movement.lotNumber}</TableCell>
                                      <TableCell className="font-medium">
                                        {movement.variantLabel || "Standard"}
                                        {movement.variantCode ? <span className="block text-xs text-muted-foreground">{movement.variantCode}</span> : null}
                                      </TableCell>
                                      <TableCell>
                                        <Badge
                                          className={
                                      ...
                                      <TableCell className="text-right font-medium">
                                        {movement.quantitySigned > 0 ? "+" : ""}
                                        {movement.quantitySigned}
                                      </TableCell>
                                      <TableCell className="text-right">{movement.balanceBefore}</TableCell>
