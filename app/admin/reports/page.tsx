...
import { backendRequest, getStoredToken } from "@/app/services/backend"
...
export default function ShopReportsPage() {
  ...
  const buildPrintableReportHtml = async () => {
  ...
  const generatePrintableReport = async () => {
    const html = await buildPrintableReportHtml()
  ...
  async function downloadReportPdf() {
    ...
    if (path) {
      ...
      try {
        ...
        console.warn('Direct PDF export failed, falling back to HTML PDF renderer:', err)
      } finally {
        setLoading(false)
      }
    }
    ...
    try {
      const html = await buildPrintableReportHtml()
      await postHtmlToPdf(html, `rapport_${selectedTab}_${format(startDate, 'yyyy-MM-dd')}.pdf`)
    } catch (err: any) {
